---
title: "Prostate Cancer ~ Predictdb eQTL&sQTL from Prostate & Testis & Lung"
...
weights <- readRDS(system.file("extdata/project_data", "European.prostate_testis_lung.weights.RDS", package = "ctwas"))
ctwas_res <- readRDS(system.file("extdata/project_data", "European.prostate_testis_lung.ctwas_noLD_allchrs.RDS", package = "ctwas"))
gwas_n <- 726828
finemap_res <- readRDS("~/Desktop/Prostate Cancer Project/European.prostate_testis_lung.finemap_res.RDS")
finemap_res_sub <- subset(finemap_res, group != "SNP" & gene_type == "protein_coding" & susie_pip > 0.8 & cs_index != 0)
```

We present a sample cTWAS report based on real data analysis. The analyzed trait is prostate cancer, the prediction models are Prostate & Testis & Lung gene expression and splicing models trained on GTEx v8 in the PredictDB format.
...
The prediction models used in this analysis are prostate & testis & lung gene expression and splicing models, trained on GTEx v8 in the PredictDB format. These models can be downloaded from https://predictdb.org/post/2021/07/21/gtex-v8-models-on-eqtl-and-sqtl/
