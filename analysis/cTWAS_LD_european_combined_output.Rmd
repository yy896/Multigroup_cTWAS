---
title: "Prostate Cancer ~ Predictdb eQTL&sQTL from Prostate & Testis & Lymphocytes & RefZ prostate cohort"
...
weights <- readRDS(system.file("extdata/project_data", "European.mashr_RefZ.weights.RDS", package = "ctwas"))
ctwas_res <- readRDS(system.file("extdata/project_data", "European.combined_weights.ctwas_LD_allchrs.RDS", package = "ctwas"))
gwas_n <- 726828
finemap_res <- readRDS("~/Desktop/Prostate Cancer Project/European.combined_weights.with_LD.finemap_res.RDS")
susie_alpha_res <- readRDS("~/Desktop/Prostate Cancer Project/European.combined_weights.with_LD.susie_alpha_res.RDS")
...
`mapping_table` is a combination of expression, splicing models and RefZ expression data.
...
```{r Ideogram, eval=TRUE, echo=FALSE, fig.height=22.5, fig.width=40, message=FALSE, warning=FALSE}
library(karyoploteR)
library(biomaRt)
library(regioneR)
...
## GSEA msigdb database

```{r pathway plot, eval=TRUE, echo=FALSE, fig.height=6, fig.width=16, message=FALSE, warning=FALSE}
library(GO.db)
library(Seurat)
library(dplyr)
library(tibble)
...
all_gene_sets_C2 <- msigdbr::msigdbr(species = "Homo sapiens",category = "C2")
apoptosis_gene_sets <- all_gene_sets_C2[grep(pattern = "APOPTOSIS",x = all_gene_sets_C2$gs_name,ignore.case = TRUE),]
nfkb_gene_sets <- all_gene_sets_C2[grep(pattern = "NFKB",x = all_gene_sets_C2$gs_name,ignore.case = TRUE),]
mapk_gene_sets <- all_gene_sets_C2[grep(pattern = "MAPK",x = all_gene_sets_C2$gs_name,ignore.case = TRUE),]
signaling_gene_sets <- all_gene_sets_C2[grep(pattern = "SIGNALING",x = all_gene_sets_C2$gs_name,ignore.case = TRUE),]

apoptosis_pathway <- split(x = apoptosis_gene_sets$gene_symbol,f=apoptosis_gene_sets$gs_name)
nfkb_pathway <- split(x = nfkb_gene_sets$gene_symbol,f=nfkb_gene_sets$gs_name)
mapk_pathway <- split(x = mapk_gene_sets$gene_symbol,f=mapk_gene_sets$gs_name)
signaling_pathway <- split(x = signaling_gene_sets$gene_symbol,f=signaling_gene_sets$gs_name)

finemap_res_stat <- finemap_res %>% 
  as_tibble() %>% 
  dplyr::select(gene_name, susie_pip) %>% 
  na.omit() %>% 
  distinct() %>% 
  group_by(gene_name)
stats <- deframe(finemap_res_stat)

### grep APOPTOSIS
fgseaRes_stat <- fgsea(pathways=apoptosis_pathway, stats=stats)#, nperm=100000)
...
(gsea_plot1 <- ggplot(fgseaResTidy_stat, aes(reorder(pathway, NES), NES)) +
  geom_col(aes(fill=padj<0.05)) +
  coord_flip() +
  labs(x="Pathways", y="Normalized Enrichment Score", title="Apotosis Pathways") +
  theme_minimal() +   
...
### grep NFKB
fgseaRes_stat <- fgsea(pathways=nfkb_pathway, stats=stats)#, nperm=100000)
...
(gsea_plot2 <- ggplot(fgseaResTidy_stat, aes(reorder(pathway, NES), NES)) +
  geom_col(aes(fill=padj<0.05)) +
  coord_flip() +
  labs(x="Pathways", y="Normalized Enrichment Score", title="NFKB Pathways") +
  theme_minimal() +   
  theme(axis.text.y = element_text(size=8),
        axis.text.x = element_text(size=8),
        axis.title = element_text(size=10)) +
...
### grep MAPK
fgseaRes_stat <- fgsea(pathways=mapk_pathway, stats=stats)#, nperm=100000)
...
(gsea_plot3 <- ggplot(fgseaResTidy_stat, aes(reorder(pathway, NES), NES)) +
  geom_col(aes(fill=padj<0.05)) +
  coord_flip() +
  labs(x="Pathways", y="Normalized Enrichment Score", title="MAPK Pathways") +
  theme_minimal() +   
  theme(axis.text.y = element_text(size=8),
        axis.text.x = element_text(size=8),
        axis.title = element_text(size=10)) +
...
### grep SIGNALING
fgseaRes_stat <- fgsea(pathways=signaling_pathway, stats=stats)#, nperm=100000)
...
fgseaResTidy_stat <- subset(fgseaResTidy_stat, padj <= 0.2)

(gsea_plot4 <- ggplot(fgseaResTidy_stat, aes(reorder(pathway, NES), NES)) +
  geom_col(aes(fill=padj<0.05)) +
  coord_flip() +
  labs(x="Pathways", y="Normalized Enrichment Score", title="Signaling Pathways") + 
  theme_minimal())
```
