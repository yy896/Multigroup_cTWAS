---
title: "Prostate Cancer ~ Predictdb eQTL&sQTL from Prostate & Testis & Lymphocytes & Lung"
...
weights <- readRDS(system.file("extdata/project_data", "European.prostate_testis_lymphocytes_lung.weights.RDS", package = "ctwas"))
ctwas_res <- readRDS(system.file("extdata/project_data", "European.prostate_testis_lymphocytes_lung.ctwas_noLD_allchrs.RDS", package = "ctwas"))
gwas_n <- 726828
finemap_res <- readRDS("~/Desktop/Prostate Cancer Project/European.prostate_testis_lymphocytes_lung.finemap_res.RDS")
finemap_res_sub <- subset(finemap_res, group != "SNP" & gene_type == "protein_coding" & susie_pip > 0.8 & cs_index != 0)
```

We present a sample cTWAS report based on real data analysis. The analyzed trait is prostate cancer, the prediction models are Prostate & Testis & Lymphocytes & Lung gene expression and splicing models trained on GTEx v8 in the PredictDB format.
...
The prediction models used in this analysis are prostate & testis & lymphocytes & lung gene expression and splicing models, trained on GTEx v8 in the PredictDB format. These models can be downloaded from https://predictdb.org/post/2021/07/21/gtex-v8-models-on-eqtl-and-sqtl/
...
weight_lung_expression_file <- system.file("extdata/project_data", 
                                           "mashr_Lung_e.db", package = "ctwas")
weight_lung_splicing_file <- system.file("extdata/project_data", 
                                         "mashr_Lung_s.db", package = "ctwas")
...
weights_lung_expression <- preprocess_weights(weight_lung_expression_file,
                                              region_info,
                                              gwas_snp_ids = z_snp$id,
                                              snp_map = snp_map,
                                              type = "expression",
                                              context = "lung",
                                              weight_name = "lung_expression",
                                              weight_format = "PredictDB",
                                              drop_strand_ambig = TRUE,
                                              scale_predictdb_weights = TRUE,
                                              load_predictdb_LD = TRUE,
                                              filter_protein_coding_genes = TRUE,
                                              ncore = 6)
weights_lung_splicing <- preprocess_weights(weight_lung_splicing_file,
                                            region_info,
                                            gwas_snp_ids = z_snp$id,
                                            snp_map = snp_map,
                                            type = "splicing",
                                            context = "lung",
                                            weight_name = "lung_splicing",
                                            weight_format = "PredictDB",
                                            drop_strand_ambig = TRUE,
                                            scale_predictdb_weights = TRUE,
                                            load_predictdb_LD = TRUE,
...
weights <- c(
  ...
  weights_lung_expression,
  weights_lung_splicing)
```
...
make_convergence_plots(param, gwas_n, colors = c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7", "#999999", "red"))
```
